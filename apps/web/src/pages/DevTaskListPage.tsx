...
export function DevTaskListPage() {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h2 className="page-title">对话与任务 · 开发 ({total})</h2>
        <Link to="/dev/tasks/new" className="btn btn-primary" style={{ textDecoration: 'none' }}>
          新建任务
        </Link>
      </div>
      {loading ? (
            ...
            <tbody>
              {tasks.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>
                    <Link to={`/dev/tasks/${t.id}`}>查看</Link>
                  </td>
                </tr>
              ))}
            </tbody>
